...
@section('content')
              ...
              <div class="col-12">
                <div class="card">
                  <div class="card-header">
                    <h3 class="card-title">Data Fasilitasi @if($date_start&&$date_end) : {{ Carbon\Carbon::parse($date_start)->translatedFormat('d F Y') }} Sampai Dengan {{ Carbon\Carbon::parse($date_end)->translatedFormat('d F Y') }}@endif @if($month) Bulan {{ Carbon\Carbon::createFromDate(null,$month,1)->translatedFormat('F') }}  @endif @if($year) Tahun {{ $year }}  @endif </h3>
                  </div>
                    ...
                    <div class="d-flex">
                      <div class="text-muted">
                        ...
                        <div class="mx-2 d-inline-block">
                          ...
                          <form action="{{ url('/fasilitasi')}}" method="POST">
                            @csrf
                      ...
                      <tbody class="font-monospace fs-5" >
                       @php
                        ...
                        <tr>
                          ...
                          <td>{{ $item->tempat }}</a></td>
                          <td>{{ $item->fasilitasi }}</></td>
                          <td>{{ $item->permasalahan }}</a></td>
                          <td>{{ $item->keterangan }}</a></td>
                          <td><a href="{{ url('/fasilitasi/'.$item->id.'/edit')}}" class="btn btn-sm btn-outline-info btn-pill">Edit</a></td>
                        </tr>
                        @endforeach
                        @if($items->count() == 0)
                        <tr >
              ...
              <div class="modal  fade" id="modal-team" tabindex="-1" role="dialog" aria-hidden="true">
                <form method="post" action="{{ url('/fasilitasi/import_excel')}}" enctype="multipart/form-data">
                  {{ csrf_field() }}
                <div class="modal-dialog modal-dialog-centered" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">Impor Data Fasilitasi</h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                      <div>
                        <label class="form-label">File Data Fasilitasi</label>
                          ...
                          <div class="card-body">
                            <div class="tab-content">
                              <div class="tab-pane fade active show" id="tabs-home-8" role="tabpanel">
                                <h4>Pilih Tanggal :</h4>
                                <form method="post" action="{{ url('/fasilitasi')}}" enctype="multipart/form-data">
                                  @csrf
