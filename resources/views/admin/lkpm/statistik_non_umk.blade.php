              ...
              <div class="card">
                <div class="card-header d-flex align-items-center justify-content-between">
                  ...
                  @if($rowsByStatusPm->count() > 0)
                    <a href="{{ route('lkpm.statistik.non-umk.kbli.export', array_merge(request()->query(), ['status' => $statusPm])) }}" class="btn btn-sm btn-success" target="_blank">
                    ...
                    <tbody>
                        ...
                        @foreach($jenisGrouped as $jenis => $jenisRows)
                          ...
                          @if($jenisRows->count() > 1)
                            <tr class="table-light fw-semibold" style="font-size: 0.9rem;">
                              ...
                              <td class="text-end">{{ number_format($jenisRows->sum('total_tenaga_kerja_wni'), 0, ',', '.') }}</td>
                              <td class="text-end">{{ number_format($jenisRows->sum('total_tenaga_kerja_wna'), 0, ',', '.') }}</td>
                      ...
                      @if($rowsByStatusPm->count() > 0)
                        ...
                        @foreach($jenisOrder as $jenis)
                          @if(isset($totalByJenis[$jenis]))
                            <tr class="table-warning fw-semibold" style="font-size: 0.9rem;">
                              ...
                              <td class="text-end">{{ number_format($totalByJenis[$jenis]->sum('total_tenaga_kerja_wni'), 0, ',', '.') }}</td>
                              <td class="text-end">{{ number_format($totalByJenis[$jenis]->sum('total_tenaga_kerja_wna'), 0, ',', '.') }}</td>
                        ...
                        <tr class="table-active fw-bold">
                          ...
                          <td class="text-end">{{ number_format($rowsByStatusPm->sum('total_tenaga_kerja_wni'), 0, ',', '.') }}</td>
                          <td class="text-end">{{ number_format($rowsByStatusPm->sum('total_tenaga_kerja_wna'), 0, ',', '.') }}</td>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const statusExportBaseUrl = {!! json_encode(route('lkpm.statistik.non-umk.rincian.export')) !!};
  const kbliExportBaseUrl = {!! json_encode(route('lkpm.statistik.non-umk.rincian.export')) !!};
  const activeFilters = {
  ...
  if (modal) {
    ...
    const statusExportLink = modal.querySelector('[data-role="status-export-link"]');
