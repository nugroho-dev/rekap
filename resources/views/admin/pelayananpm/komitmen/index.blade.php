...
@section('content')
                    ...
                    <div class="row g-2 align-items-center">
                      ...
                      <div class="col-auto ms-auto d-print-none">
                        <div class="btn-list">
                          ...
                          <a href="{{ url('/commitment/statistik')}}" class="btn btn-info d-none d-sm-inline-block">
                          ...
                          <a href="{{ url('/commitment/statistik')}}" class="btn btn-info d-sm-none btn-icon">
                            <!-- Download SVG icon from http://tabler-icons.io/i/plus --> 
                            <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-chart-infographic"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7m-4 0a4 4 0 1 0 8 0a4 4 0 1 0 -8 0" /><path d="M7 3v4h4" /><path d="M9 17l0 4" /><path d="M17 14l0 7" /><path d="M13 13l0 8" /><path d="M21 12l0 9" /></svg>
                          </a>
                        </div>
              ...
              <div class="col-12">
                <div class="card">
                  <div class="card-header">
                    <h3 class="card-title">Data Komitmen @if($date_start&&$date_end) : {{ Carbon\Carbon::parse($date_start)->translatedFormat('d F Y') }} Sampai Dengan {{ Carbon\Carbon::parse($date_end)->translatedFormat('d F Y') }}@endif @if($month) Bulan {{ Carbon\Carbon::createFromDate(null,$month,1)->translatedFormat('F') }}  @endif @if($year) Tahun {{ $year }}  @endif </h3>
                  </div>
                    ...
                    <div class="d-flex">
                      <div class="text-muted">
                        ...
                        <div class="mx-2 d-inline-block">
                          ...
                          <form action="{{ url('/komitmensort')}}" method="POST">
                            @csrf
                  ...
                  <div class="table-responsive">
                    <table class="table card-table table-vcenter text-nowrap  table-striped ">
                      <thead class="text-center">
                        <tr>
                          <th class="w-1" >No.</th>
                          <th class="w-1" >Nama Pelaku Usaha</th>
                          <th >Alamat Pelaku Usaha</th>
                          <th >Nomor Induk Berusaha</th>
                          <th class="w-1" >Nama Proyek</th>
                          <th >Jenis Izin</th>
                          <th >Status</th>
                          <th class="w-1" >Tanggal Izin Terbit</th>
                          <th >Keterangan</th>
                          <th >*</th>
                        </tr>
                      </thead>
                      <tbody class="font-monospace fs-5" >
                       @php
                        ...
                        <tr>
                          ...
                          <td class="text-wrap text-justify">{{ $item->alamat_pelaku_usaha }}</td>
                          <td class="text-center">{{ $item->nib }}</td>
                          <td class="text-center">{{ $item->nama_proyek }}</td>
                          <td class="text-wrap text-justify ">{{ $item->jenis_izin }}</td>
                          <td class="text-center">{{ $item->status }}</td>
                          <td class="text-center">{{ Carbon\Carbon::parse($item->tanggal_izin_terbit)->translatedFormat('d F Y') }}</td>
                          <td class="text-center text-wrap">{{ $item->keterangan }}</td>
              ...
              <div class="modal  fade" id="modal-team" tabindex="-1" role="dialog" aria-hidden="true">
                <form method="post" action="{{ url('/commitment/import_excel')}}" enctype="multipart/form-data">
                  {{ csrf_field() }}
                <div class="modal-dialog modal-dialog-centered" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">Impor Data Komitmen</h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                      <div>
                        <label class="form-label">File Data Komitmen</label>
                          ...
                          <div class="card-body">
                            <div class="tab-content">
                              <div class="tab-pane fade active show" id="tabs-home-8" role="tabpanel">
                                <h4>Pilih Tanggal :</h4>
                                <form method="post" action="{{ url('/komitmensort')}}" enctype="multipart/form-data">
                                  @csrf
...
<script>
    $(document).ready(function() {
        $('.openModal').on('click', function() {
            ...
            $.ajax({
                url: `{{ url('/komitmen')}}/${userId}`, // Endpoint resource controller
                type: 'GET',
        ...
        $('.openModalDel').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                  const dataSlug = data.slug;
                  if (data.slug) {
                        $('#delLoi').attr('action', `/komitmen/${dataSlug}`);
                        $('#userName').text(data.nama_expo);
