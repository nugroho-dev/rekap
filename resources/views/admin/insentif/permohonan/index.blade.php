...
@section('content')
                    ...
                    <div class="row g-2 align-items-center">
                      ...
                      <div class="col-auto ms-auto d-print-none">
                        <div class="btn-list">
                          ...
                          <a href="{{ url('/insentif/statistik')}}" class="btn btn-info d-none d-sm-inline-block">
                          ...
                          <a href="{{ url('/insentif/statistik')}}" class="btn btn-info d-sm-none btn-icon">
                            <!-- Download SVG icon from http://tabler-icons.io/i/plus --> 
                            <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-chart-infographic"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7m-4 0a4 4 0 1 0 8 0a4 4 0 1 0 -8 0" /><path d="M7 3v4h4" /><path d="M9 17l0 4" /><path d="M17 14l0 7" /><path d="M13 13l0 8" /><path d="M21 12l0 9" /></svg>
                          </a>
                        </div>
                      ...
                      <div class="col-auto ms-auto d-print-none">
                        <div class="btn-list">
                          ...
                          <a href="{{ url('/insentif/create')}}" class="btn btn-primary d-none d-sm-inline-block" >
                            <!-- Download SVG icon from http://tabler-icons.io/i/plus --> 
                          ...
                          <a href="{{ url('/insentif/create')}}" class="btn btn-primary d-sm-none btn-icon">
                            <!-- Download SVG icon from http://tabler-icons.io/i/plus --> 
                            <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-table-import"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 21h-7a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h14a2 2 0 0 1 2 2v8" /><path d="M3 10h18" /><path d="M10 3v18" /><path d="M19 22v-6" /><path d="M22 19l-3 -3l-3 3" /></svg>
                          </a>
                        </div>
                    ...
                    <div class="d-flex">
                      <div class="text-muted">
                        ...
                        <div class="mx-2 d-inline-block">
                          ...
                          <form action="{{ url('/insentif')}}" method="GET">
                      ...
                      <tbody class="font-monospace fs-5" >
                       @php
                        ...
                        <tr >
                          ...
                          <td class="align-top">
                            <div class="text-capitalize">{{ $item->penerima }}</div>
                            <div class="text-secondary">{{ $item->alamat_penerima }}</div>
                            <div class="text-secondary ">{{ $item->jenis_perusahaan }}</div>
                            <div class="text-secondary">{{ $item->nama_perusahaan }}</div>
                            <div class="text-secondary ">{{ $item->alamat_perusahaan }}</div>
                           
                          </td>
                          <td class="align-top">
                            <div> {{ \Carbon\Carbon::parse($item->tanggal_permohonan)->translatedFormat('D, d M Y')}}</div>
                            <div class="text-secondary text-wrap">{!! $item->bentuk_pemberian !!}</div>
                            <div class="text-secondary text-wrap">{{ $item->pemberian_insentif }} Sejumlah {{ $item->persentase_insentif }}</div>
                            <div class="text-secondary"></div>
                          </td>
                          <td class="align-top">
                            <div>{{ $item->no_sk }}</div>
                            <div class="text-secondary">{{ $item->no_rekomendasi }}</div>
                            <div class="text-secondary"> {{ \Carbon\Carbon::parse($item->tanggal_sk )->translatedFormat('D, d M Y')}}</div>
                          </td>
                          <td>
                            <span class="dropdown">
                              ...
                              <div class="dropdown-menu dropdown-menu-end">
                                <a class="dropdown-item" href="{{ url('/insentif/'.$item->slug.'/edit')}}" >
                                  Edit
                                </a>
                                <button class="dropdown-item openModalIdentitas" data-id="{{ $item->slug }}">
                                  Surat Keputusan
                                </button>
                                <button class="dropdown-item openModalAduan" data-id="{{ $item->slug }}">
                                  File Permohonan
                                </button>
              ...
              <div class="modal  fade" id="modal-team" tabindex="-1" role="dialog" aria-hidden="true">
                <form method="post" action="{{ url('/insentif/import_excel')}}" enctype="multipart/form-data">
                  {{ csrf_field() }}
                <div class="modal-dialog modal-dialog-centered" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title">Impor Data Insentif</h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                      <div>
                        <label class="form-label">File Data Insentif</label>
                          ...
                          <div class="card-body">
                            <div class="tab-content">
                              <div class="tab-pane fade active show" id="tabs-home-8" role="tabpanel">
                                <h4>Pilih Tanggal :</h4>
                                <form method="get" action="{{ url('/insentif')}}" enctype="multipart/form-data">
                ...
                <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
                  <div class="modal-content">
                    ...
                    <div class="modal-body text-center py-4">
                      ...
                      <h3>Anda yakin ?</h3>
                      <div class="text-secondary">Hapus Data Permohonan Insentif <span id="userName"></span> !</div>
                    </div>
                    <div class="modal-footer">
                      <div class="w-100">
                        <div class="row">
        ...
        $('.openModalDel').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                  const dataSlug = data.slug;
                  if (data.slug) {
                        $('#delLoi').attr('action', `/insentif/${dataSlug}`);
                        $('#userName').text(data.penerima);
