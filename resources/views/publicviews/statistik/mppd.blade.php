...
<style>
  ...
  .profesi-list { list-style: none; padding: 0; margin: 0; }
  .profesi-list li { display:flex; justify-content:space-between; gap:.75rem; padding:.5rem 0; border-bottom: 1px solid rgba(226,232,240,0.6); transition: background 0.2s ease; }
  .profesi-list li:hover { background: rgba(139,92,246,0.03); padding-left: .5rem; padding-right: .5rem; border-radius: 6px; }
  ...
  <div class="card stats-card">
    <div class="card-body">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
          <h3 class="mb-0">Statistik MPP Digital</h3>
          <div class="small-muted">Ringkasan data izin profesi kesehatan</div>
        </div>
        <div class="text-end">
          <div class="h4 mb-0">Total: {{ number_format($total ?? 0) }}</div>
          <div class="small-muted">Izin tercatat</div>
        </div>
      </div>
      ...
      <div class="row g-3 mb-3">
        <div class="col-md-8 col-sm-12">
          <div class="mini-card">
            <div class="d-flex justify-content-between align-items-center mb-2">
              <h5 class="mb-0">Jumlah Izin per Bulan</h5>
                    ...
                    <tr>
                      ...
                      <td class="text-center">
                        <button type="button" class="btn btn-sm btn-outline-primary btn-daily"
                          ...
                          data-days-detail='@json($profesiDailyByMonth[$m] ?? [])'>
                          Lihat
                        </button>
                      </td>
                    </tr>
        ...
        <div class="col-md-4 col-sm-12">
          ...
          <div class="mini-card">
            <div class="muted mb-2">Top Profesi (Izin Terbit)</div>
            <ul class="profesi-list mb-0">
              @foreach($stats->take(12) as $row)
                <li>
                  <div style="flex:1">{{ \Illuminate\Support\Str::limit($row->profesi ?? 'Tidak Diketahui', 60) }}</div>
            ...
            <div class="mt-2 text-center">
              <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#profesiModal">Selengkapnya</button>
              <button type="button" class="btn btn-sm btn-outline-primary ms-2" data-bs-toggle="modal" data-bs-target="#profesiPerBulanModal">Profesi per Bulan</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="row g-3 mb-3 mt-2">
        <div class="col-12">
          <div class="mini-card">
            <div class="d-flex justify-content-between align-items-center mb-2">
              <div class="muted">Jumlah Izin per Tahun</div>
      ...
      <div class="row g-3 mb-3">
        <div class="col-md-12 col-sm-12">
          <div class="mini-card">
            <div class="d-flex justify-content-between align-items-center mb-2">
              <div class="muted">Komposisi Profesi per Tahun</div>
            </div>
            <div id="chart-stacked-profesi" style="height:300px"></div>
          </div>
        </div>
      </div>
      ...
      <div class="row g-3 mb-3">
        <div class="col-md-6 col-sm-12">
          <div class="mini-card">
            <div class="muted">Top Profesi (Donut)</div>
            <div id="chart-top-profesi" style="height:260px"></div>
          </div>
        </div>
        <div class="col-md-6 col-sm-12">
          <div class="mini-card">
            <div class="muted">Kumulatif per Profesi (Area)</div>
...
<!-- Daily profesi breakdown modal -->
<div class="modal fade" id="dailyProfesiModal" tabindex="-1" aria-labelledby="dailyProfesiModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="dailyProfesiModalLabel">Rincian Profesi</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div id="dailyProfesiMeta" class="mb-2 small-muted"></div>
        <div class="table-responsive">
...
<!-- Profesi full modal -->
<div class="modal fade" id="profesiModal" tabindex="-1" aria-labelledby="profesiModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="profesiModalLabel">Daftar Profesi (Izin Terbit)</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="table-responsive">
            ...
            <tbody>
              @forelse($stats as $row)
                <tr>
                  ...
                  <td>{{ $row->profesi ?? 'Tidak Diketahui' }}</td>
              ...
              @empty
                <tr>
                  <td colspan="4" class="text-center small-muted">Belum ada data izin</td>
                </tr>
              @endforelse
            </tbody>
          </table>
...
<div class="modal fade" id="yearlyModal" tabindex="-1" aria-labelledby="yearlyModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="yearlyModalLabel">Jumlah Izin per Tahun</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="table-responsive">
...
<!-- Profesi per bulan modal -->
<div class="modal fade" id="profesiPerBulanModal" tabindex="-1" aria-labelledby="profesiPerBulanLabel" aria-hidden="true">
  <div class="modal-dialog modal-fullscreen modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="profesiPerBulanLabel">Jumlah Profesi per Bulan</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="table-responsive">
          <table class="table table-sm table-bordered align-middle mb-0">
            ...
            <tbody>
              @php $profesiList = $allProfesi ?? []; @endphp
              @forelse($profesiList as $p)
                <tr>
                  <td class="text-center">{{ $loop->iteration }}</td>
                  <td style="min-width:260px">{{ $p ?: 'Tidak Diketahui' }}</td>
                  @php $sum = 0; @endphp
                  @foreach(range($start,$end) as $mm)
                    @php $cnt = intval($profesiByMonth[$p][$mm] ?? 0); $sum += $cnt; @endphp
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  document.querySelectorAll('.btn-daily').forEach(function(btn){
    btn.addEventListener('click', function(){
      ...
      window.currentMonthlyProfesi = daysDetail || {};
      ...
      setTimeout(function(){
        document.querySelectorAll('#dailyModalBody .day-cell').forEach(function(el){
          el.addEventListener('click', function(){
            ...
            if(!d) return;
            openDailyProfesiModal(d, monthName, year);
          });
        });
      },20);
    ...
    function openDailyProfesiModal(day, monthName, year){
      var meta = document.getElementById('dailyProfesiMeta');
      if(meta) meta.textContent = 'Tanggal: ' + (day) + ' ' + monthName + ' ' + year;

      var body = document.getElementById('dailyProfesiBody');
      var data = (window.currentMonthlyProfesi && window.currentMonthlyProfesi[day]) ? window.currentMonthlyProfesi[day] : {};
      if(!body) return;
      var rows = '';
      ...
      var modalEl = document.getElementById('dailyProfesiModal');
      if(window.bootstrap && modalEl){
        var modal = new bootstrap.Modal(modalEl);
        modal.show();
      }
  ...
  (function(){
    ...
    var profesiSeriesObj = @json($profesiSeries ?? []);
    var weekdayCounts = <?php echo json_encode($weekdayCounts ?? [0,0,0,0,0,0,0], 15, 512) ?>;
    var topProfesi = @json($topProfesi ?? []);
    var years = @json($availableYears ?? []);
    var profesiSeriesByYear = @json($profesiSeriesByYear ?? []);
    ...
    // Komposisi Profesi per Tahun: horizontal bar per profesi (total dalam 1 tahun)
    try{
      var stEl = document.getElementById('chart-stacked-profesi');
      if(stEl && window.ApexCharts){
        var profesiNames = Object.keys(profesiSeriesObj || {});
        var totals = profesiNames.map(function(k){
          var arr = profesiSeriesObj[k] || [];
          return arr.reduce(function(a,b){ return a + (parseInt(b,10)||0); }, 0);
        });
        var pairs = profesiNames.map(function(k,i){ return {k:k, total: totals[i]}; })
    ...
    // Top profesi donut (legend hidden) with data labels
    try{
      var topEl = document.getElementById('chart-top-profesi');
      if(topEl && window.ApexCharts){
        var labels = topProfesi.map(function(o){ return o.name; });
        var vals = topProfesi.map(function(o){ return o.y; });
    ...
    // Cumulative area for top 6 profesi with modern styling
    try{
      var cumEl = document.getElementById('chart-cumulative');
      if(cumEl && window.ApexCharts){
        var entries = Object.keys(profesiSeriesObj).map(function(k){
          var sum = profesiSeriesObj[k].reduce(function(a,b){return a+b;},0);
          return {k:k,total:sum};
        }).sort(function(a,b){return b.total - a.total}).slice(0,6);
        var cumSeries = entries.map(function(e){
          var arr = [];
          var s=0;
          profesiSeriesObj[e.k].forEach(function(v){ s+=v; arr.push(s); });
    ...
    // Expose helper to render daily heatmap
