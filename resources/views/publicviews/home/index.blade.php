      ...
      <div class="table-responsive">
        <table class="table card-table table-striped table-hover table-vcenter text-nowrap datatable">
          @foreach($kategori as $kat)
            <thead>
              <tr>
                <th colspan="6" class=" text-black  fs-4 py-3">{{ $kat->nama }}</th>
              </tr>
              <tr class="bg-light">
                <th class="text-uppercase">Jenis Informasi</th>
                <th class="text-uppercase text-center">Status Update</th>
                <th class="text-uppercase text-center">Tanggal Update</th>
                <th class="text-uppercase text-center">Jumlah Data</th>
                <th class="text-uppercase text-center">Data Set</th>
                <th class="text-uppercase text-center">Statistik</th>
              </tr>
            </thead>
            <tbody>
              @forelse($kat->jenisInformasi as $jenis)
                <tr>
                  ...
                  <td class="text-center"><span class="text-muted">{{ $jenis->updated_model_at ? \Carbon\Carbon::parse($jenis->updated_model_at)->format('d M Y H:i') : '-' }}</span></td>
                  <td class="text-center"><span class="fw-bold text-primary">{{ number_format($jenis->jumlah ?? 0) }} Data</span></td>
                  <td class="text-center">
                    @if($jenis->dataset)
