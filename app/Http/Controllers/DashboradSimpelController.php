...
class DashboradSimpelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Vsimpel::query();
        $date_basis = $request->input('date_basis', 'tte');
        $date_basis = in_array($date_basis, ['tte', 'rekomendasi'], true) ? $date_basis : 'tte';
        ...
        if ($request->has('date_start') && $request->has('date_end')) {
            ...
            return redirect('/simpel')->with('error', 'Silakan Cek Kembali Pilihan Range Tanggal Anda');
                ...
                $query->whereBetween($date_basis, [$date_start, $date_end]);
            }
        }
        ...
        if ($request->has('search') && $request->has('month') && $request->has('year')) {
            ...
            return redirect('/simpel')->with('error', 'Silakan Cek Kembali Pilihan Bulan dan Tahun Anda');
                ...
                $query->whereMonth($date_basis, $month)
                    ->whereYear($date_basis, $year)
        ...
        if ($request->has('year')) {
            $query->whereYear($date_basis, $year);
        }
        ...
        return view('admin.nonberusaha.simpel.index', compact('judul', 'items', 'perPage', 'search', 'date_start', 'date_end', 'month', 'year', 'date_basis'));
    }
    public function statistik(Request $request)
    {
        ...
        $date_basis = $request->input('date_basis', 'tte');
        $date_basis = in_array($date_basis, ['tte', 'rekomendasi'], true) ? $date_basis : 'tte';
        $monthExpr = 'month(' . $date_basis . ')';
        $yearExpr = 'year(' . $date_basis . ')';
        $countExpr = 'count(' . $date_basis . ')';
		$date_start = $request->input('date_start');
		$date_end = $request->input('date_end');
		$month = $request->input('month');
		$now = Carbon::now();
		...
		if ($request->has('year')) {
            ...
            $terbit = DB::table('view_simpel')
                ->selectRaw($monthExpr . ' AS bulan, ' . $yearExpr . ' AS tahun, ' . $countExpr . ' as jumlah_data, sum(jumlah_hari)as j_hari ')
                ->whereYear($date_basis, $year)
        ...
        return view('admin.nonberusaha.simpel.statistik',compact('judul','jumlah_permohonan','date_start','date_end','month','year','rataRataJumlahHariPerBulan', 'rataRataJumlahHari','totalJumlahData','totalJumlahHari','coverse','date_basis'));
	}
