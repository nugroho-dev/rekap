...
class SicantikProsesController extends Controller
{
    public function index()
    {
        ...
        $proses = Proses::where('status', '=','Proses')->orderBy('tgl_pengajuan','desc')->paginate(50);//DB::table('proses')->whereIn('jenis_proses_id', [2,18,30,40])->whereRaw('DATE(end_date) = DATE(NOW())')->whereRaw('TIME_FORMAT(end_date, "%H:%i") >= TIME_FORMAT("'.$date2.'", "%H:%i")')->whereRaw('TIME_FORMAT(end_date, "%H:%i") <= TIME_FORMAT("'.$date1.'","%H:%i")')->paginate(50);
